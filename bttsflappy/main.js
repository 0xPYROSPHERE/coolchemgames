  ...
  Call() {
    if (-1 === this._timerId) {
      ...
      a >= b && this._canRunImmediate
        ? ((this._lastCallTime = c), this._RunCallback())
...
window.DOMElementHandler = class extends self.DOMHandler {
  ...
  _OnSetVisible(c) {
    this._autoAttach &&
      (this._elementMap.get(c.elementId).style.display = c.isVisible
        ? ""
        : "none");
  }
  _OnUpdatePosition(c) {
...
("use strict");
{
  const c = /(iphone|ipod|ipad|macos|macintosh|mac os x)/i.test(
      navigator.userAgent,
    ),
  ...
  window.RuntimeInterface = class g {
    constructor(d) {
      ...
      this.AddRuntimeComponentMessageHandler(
        ...
        (k) => this._OnCordovaFetchLocalFile(k),
      );
      this.AddRuntimeComponentMessageHandler(
        "runtime",
        "create-job-worker",
        (k) => this._OnCreateJobWorker(k),
      );
      "cordova" === this._exportType
        ? document.addEventListener("deviceready", () => this._Init(d))
        : this._Init(d);
    }
    Release() {
    ...
    IsiOSWebView() {
      ...
      return (
        (c && H(this._exportType)) ||
        navigator.standalone ||
        /crios\/|fxios\/|edgios\//i.test(d)
      );
    }
    IsAndroid() {
      return a;
    ...
    async _Init(d) {
        ...
        for (let l = 0, n = d.engineScripts.length; l < n; ++l) {
          ...
          this._localFileStrings.hasOwnProperty(k)
            ...
            : this._localFileBlobs.hasOwnProperty(k) &&
              (d.engineScripts[l] = URL.createObjectURL(
                this._localFileBlobs[k],
              ));
        }
      }
      d.baseUrl
      ...
      this._messageChannelPort.onmessage = (l) =>
        this._OnMessageFromRuntime(l.data);
      ...
      this._useWorker
        ? await this._InitWorker(d, k.port2)
        : await this._InitDOM(d, k.port2);
    }
    _GetWorkerURL(d) {
      d = this._workerScriptURLs.hasOwnProperty(d)
        ? this._workerScriptURLs[d]
        : d.endsWith("/workermain.js") &&
            this._workerScriptURLs.hasOwnProperty("workermain.js")
          ? this._workerScriptURLs["workermain.js"]
          : "playable-ad" === this._exportType &&
              this._localFileBlobs.hasOwnProperty(d.toLowerCase())
            ? this._localFileBlobs[d.toLowerCase()]
            : d;
    ...
    async CreateWorker(d, k, l) {
      ...
      if ("cordova" === this._exportType && this._isFileProtocol)
        return (
          (d = await this.CordovaFetchLocalFileAsArrayBuffer(
            l.isC3MainWorker ? d : this._scriptFolder + d,
          )),
          (d = new Blob([d], {
      ...
      d = "https://cdn.jsdelivr.net/gh/bubbls/UGS-Assets@main/flappy-tower/scripts/dispatchworker.js";
    ...
    async _InitWorker(d, k) {
      ...
      if ("cordova" === this._exportType)
        for (let q = 0, t = d.projectScripts.length; q < t; ++q) {
          const x = d.projectScripts[q],
            E = x[0];
          if (
            E === d.mainProjectScript ||
            "scriptsInEvents.js" === E ||
            E.endsWith("/scriptsInEvents.js")
          )
            x[1] = await this._MaybeGetCordovaScriptURL(E);
        }
      this._worker.postMessage(
        Object.assign(this._GetCommonRuntimeOptions(d), {
          type: "init-runtime",
          isInWorker: !0,
          messagePort: k,
          canvas: l,
      ...
      self.c3_callFunction = (q, t) =>
        this._runtimeDomHandler._InvokeFunctionFromJS(q, t);
    ...
    async _InitDOM(d, k) {
      ...
      this._domHandlers = C.map((q) => new q(this));
      this._FindRuntimeDOMHandler();
      var l = d.engineScripts.map((q) =>
        "string" === typeof q ? new URL(q, this._baseUrl).toString() : q,
      );
      Array.isArray(d.workerDependencyScripts) &&
        l.unshift(...d.workerDependencyScripts);
      ...
      for (let [q, t] of r)
        if ((t || (t = q), q === n))
          try {
            ((t = await this._MaybeGetCordovaScriptURL(t)),
              await e(t),
              "preview" !== this._exportType ||
                l[q] ||
          ...
          "scriptsInEvents.js" === q ||
          q.endsWith("/scriptsInEvents.js")
        )
          ((t = await this._MaybeGetCordovaScriptURL(t)), await e(t));
      "preview" === this._exportType &&
      "object" !== typeof self.C3.ScriptsInEvents
        ...
        : ((d = Object.assign(this._GetCommonRuntimeOptions(d), {
            isInWorker: !1,
            messagePort: k,
            canvas: this._canvas,
          ...
          (this._localRuntime = self.C3_CreateRuntime(d)),
          await self.C3_InitRuntime(this._localRuntime, d));
    }
    _ReportProjectMainScriptError(d, k) {
      this._RemoveLoadingMessage();
      console.error(`[Preview] Failed to load project main script (${d}): `, k);
      alert(
        `Failed to load project main script (${d}). Check all your JavaScript code has valid syntax. Press F12 and check the console for error details.`,
      );
    }
    ...
    _RemoveLoadingMessage() {
      ...
      d &&
        (d.parentElement.removeChild(d), (window.cr_previewLoadingElem = null));
    }
    ...
    ["_OnMessageFromRuntime"](d) {
      ...
      if ("result" === k) this._OnResultFromRuntime(d);
        ...
        (this._RemoveLoadingMessage(), alert(d.message));
    ...
    _OnEventFromRuntime(d) {
      ...
      if ((d = F.get(k)))
        if ((d = d.get(l))) {
          ...
          q && q.then
            ? q
                .then((t) => this._PostResultToRuntime(r, !0, t))
                .catch((t) => {
    ...
    AddRuntimeComponentMessageHandler(d, k, l) {
      ...
      if (n.has(k))
        throw Error(`[DOM] Component '${d}' already has handler '${k}'`);
      n.set(k, l);
    }
    ...
    async _WasmDecodeWebMOpus(d) {
      d = await this.PostToRuntimeComponentAsync(
        "runtime",
        "opus-decode",
        {
          arrayBuffer: d,
    ...
    async _MaybeGetCordovaScriptURL(d) {
      return "cordova" === this._exportType &&
        (d.startsWith("file:") ||
          (this._isFileProtocol && this.IsRelativeURL(d)))
        ? (d.startsWith(this._baseUrl) && (d = d.substr(this._baseUrl.length)),
          (d = await this.CordovaFetchLocalFileAsArrayBuffer(d)),
          (d = new Blob([d], {
            type: "application/javascript",
          })),
    ...
    async RequestPermissions(d) {
      ...
      for (const k of d)
        if (
          !(await this._HasPermission(k)) &&
          !1 === (await this._RequestPermission(k))
        )
          return !1;
      return !0;
    }
    async RequirePermissions(...d) {
      if (!1 === (await this.RequestPermissions(d)))
        throw Error("Permission not granted");
    }
    CordovaFetchLocalFile(d) {
      const k =
        window.cordova.file.applicationDirectory + "www/" + d.toLowerCase();
      return new Promise((l, n) => {
    ...
    _SendWrapperMessage(d) {
      if ("windows-webview2" === this._exportType)
        ...
        window.webkit.messageHandlers.C3Wrapper.postMessage(JSON.stringify(d));
    ...
    async _ConvertDataUrisToBlobs() {
      ...
      for (const [k, l] of Object.entries(this._localFileBlobs))
        d.push(this._ConvertDataUriToBlobs(k, l));
      await Promise.all(d);
    }
    async _ConvertDataUriToBlobs(d, k) {
      if ("object" === typeof k)
        ((this._localFileBlobs[d] = new Blob([k.str], {
          type: k.type,
        })),
...
("use strict");
{
  ...
  function H() {
    ...
    return "textarea" === g
      ? !0
      : "input" === g
        ? d.has(f.type.toLowerCase() || "text")
        : w(f);
  }
  c.AddDOMHandlerClass(
    class extends self.DOMHandler {
      constructor(f) {
        ...
        f.AddRuntimeComponentMessageHandler(
          "runtime",
          "raster-svg-image",
          (k) => this._OnRasterSvgImage(k),
        );
        f.AddRuntimeComponentMessageHandler(
          "runtime",
          "get-svg-image-size",
          (k) => this._OnGetSvgImageSize(k),
        );
        f.AddRuntimeComponentMessageHandler(
          "runtime",
          "set-target-orientation",
          (k) => this._OnSetTargetOrientation(k),
        );
        f.AddRuntimeComponentMessageHandler("runtime", "register-sw", () =>
          this._OnRegisterSW(),
        );
        f.AddRuntimeComponentMessageHandler(
          "runtime",
        ...
        f.AddRuntimeComponentMessageHandler("runtime", "alert", (k) =>
          this._OnAlert(k),
        );
        f.AddRuntimeComponentMessageHandler(
          "runtime",
        ...
        window.addEventListener("fullscreenerror", (k) =>
          this._OnFullscreenError(k),
        );
        window.addEventListener("webkitfullscreenerror", (k) =>
          this._OnFullscreenError(k),
        );
        window.addEventListener("mozfullscreenerror", (k) =>
          this._OnFullscreenError(k),
        );
        f.IsiOSWebView() &&
      ...
      Attach() {
        ...
        window.addEventListener("keydown", (g) =>
          this._OnKeyEvent("keydown", g),
        );
        window.addEventListener("keyup", (g) => this._OnKeyEvent("keyup", g));
        window.addEventListener("dblclick", (g) =>
          this._OnMouseEvent("dblclick", g, e),
        );
        window.addEventListener("wheel", (g) =>
          this._OnMouseWheelEvent("wheel", g),
        );
        "undefined" !== typeof PointerEvent
          ...
          : (window.addEventListener("mousedown", (g) => {
            ...
            window.addEventListener("mouseup", (g) =>
              this._OnMouseEventAsPointer("pointerup", g),
            ),
            ...
            window.addEventListener("touchmove", (g) =>
              this._OnTouchEvent("pointermove", g),
            ),
            window.addEventListener("touchend", (g) =>
              this._OnTouchEvent("pointerup", g),
            ),
            window.addEventListener("touchcancel", (g) =>
              this._OnTouchEvent("pointercancel", g),
            ));
      ...
      _OnAndroidVirtualKeyboardChange(f, g) {
        ...
        if (0 < g) {
          ...
          d &&
            ...
            (f = (d.top + d.bottom) / 2 - (f - g) / 2),
            f > g && (f = g),
            0 > f && (f = 0),
            0 < f && (document.body.style.transform = `translateY(${-f}px)`));
        }
      }
      ...
      _OnFullscreenChange() {
        const f = c.IsDocumentFullscreen();
        f &&
      ...
      _OnPointerEvent(f, g) {
        ...
        "mouse" === g.pointerType &&
          ...
          "pointerdown" === f
            ? (d = "mousedown")
            : "pointerup" === f && (d = "mouseup"),
          this._OnMouseEvent(d, g, h),
          (this._mousePointerLastButtons = g.buttons));
      }
      ...
      _PlayPendingMedia() {
        ...
        if (!this._isSilent)
          for (const g of f)
            (f = g.play()) &&
              f.catch((d) => {
                this._mediaRemovedPendingPlay.has(g) ||
                  this._mediaPendingPlay.add(g);
              });
      }
      TryPlayMedia(f) {
        ...
        g &&
          g.catch((d) => {
            this._mediaRemovedPendingPlay.has(f) ||
              this._mediaPendingPlay.add(f);
          });
      }
      RemovePendingPlay(f) {
      ...
      _OnPostToDebugger(f) {
        window.c3_postToMessagePort &&
          ((f.from = "runtime"), window.c3_postToMessagePort(f));
      }
      ...
      _OnWrapperMessage(f) {
        "entered-fullscreen" === f
          ? (c._SetWrapperIsFullscreenFlag(!0), this._OnFullscreenChange())
          : "exited-fullscreen" === f
            ? (c._SetWrapperIsFullscreenFlag(!1), this._OnFullscreenChange())
            : console.warn("Unknown wrapper message: ", f);
      }
    },
  );
}
...
self.JobSchedulerDOM = class {
  constructor(c) {
    ...
    this._baseUrl = c.GetBaseURL();
    "preview" === c.GetExportType()
      ? (this._baseUrl += "workers/")
      : (this._baseUrl += c.GetScriptFolder());
...
window.C3_IsSupported &&
  (window.c3_runtimeInterface = new self.RuntimeInterface({
    ...
    engineScripts: [
      "https://cdn.jsdelivr.net/gh/bubbls/UGS-Assets@main/flappy-tower/scripts/c3runtime.js",
    ],
    projectScripts: [],
    mainProjectScript: "",
    scriptFolder:
      "https://cdn.jsdelivr.net/gh/bubbls/UGS-Assets@main/flappy-tower/scripts/",
...
("use strict");
{
  ...
  self.AudioDOMHandler = class extends self.DOMHandler {
    constructor(a) {
      ...
      self.C3Audio_OnMicrophoneStream = (b, e) =>
        this._OnMicrophoneStream(b, e);
    ...
    *audioInstancesByTag(a) {
    ...
    async _GetAudioInstance(a, b, e, h, m) {
      for (const p of this._audioInstances)
        if (p.GetUrl() === b && (p.CanBeRecycled() || m))
          return (p.SetTag(h), p);
        ...
        ((this._lastAudioInstance = await this._GetAudioInstance(
          ...
          v
            ? (this._lastAudioInstance.SetPannerEnabled(!0),
              ...
              v.hasOwnProperty("uid") && this._lastAudioInstance.SetUID(v.uid))
            : this._lastAudioInstance.SetPannerEnabled(!1),
          this._lastAudioInstance.Play(u, z, B, w));
      } catch (A) {
    ...
    _OnTick(a) {
      ...
      for (const e of a.instPans) {
        ...
        for (const h of this._audioInstances)
          h.GetUID() === a && h.SetPanXYA(e.x, e.y, e.angle);
      }
    }
    async _AddEffect(a) {
      var b = a.type;
    ...
    _SetEffectParam(a) {
      ...
      !a ||
        0 > b ||
        b >= a.length ||
        (a[b].SetParam(e, h, m, p), this._PostUpdatedFxState());
    }
    _RemoveEffects(a) {
      a = a.tag.toLowerCase();
...
self.C3AudioBuffer = class {
  ...
  static Create(c, a, b, e, h) {
    ...
    return !h || c.IsPlayMusicAsSound() || m
      ? new self.C3WebAudioBuffer(c, a, b, e, h, m)
      : new self.C3Html5AudioBuffer(c, a, b, e, h);
  }
  CreateInstance(c) {
    return "html5" === this._api
      ? new self.C3Html5AudioInstance(this._audioDomHandler, this, c)
      : new self.C3WebAudioInstance(this._audioDomHandler, this, c);
  }
  _Load() {}
  Load() {
...
self.C3WebAudioBuffer = class extends self.C3AudioBuffer {
  ...
  async _Fetch() {
    ...
    if (
      "cordova" === c.GetExportType() &&
      c.IsRelativeURL(this._url) &&
      c.IsFileProtocol()
    )
      this._audioData = await c.CordovaFetchLocalFileAsArrayBuffer(this._url);
    else {
      c = await fetch(this._url);
      if (!c.ok)
        throw Error(`error fetching audio data: ${c.status} ${c.statusText}`);
...
self.C3Html5AudioInstance = class extends self.C3AudioInstance {
  ...
  SetLooping(c) {
    c = !!c;
    this._isLooping !== c &&
      ((this._isLooping = c), (this.GetAudioElement().loop = c));
  }
  _UpdatePlaybackRate() {
...
self.C3WebAudioInstance = class extends self.C3AudioInstance {
  ...
  SetLooping(c) {
    ...
    this._isLooping !== c &&
      ((this._isLooping = c),
      this._bufferSource && (this._bufferSource.loop = c));
  }
  _UpdatePlaybackRate() {
    let c = this._playbackRate;
    this._isTimescaled && (c *= this._audioDomHandler.GetTimeScale());
  ...
  Seek(c) {
    this._isStopped ||
      this.HasEnded() ||
      (this._isPaused
        ? (this._resumePosition = c)
        : (this.Pause(), (this._resumePosition = c), this.Resume()));
  }
  GetResumePosition() {
  ...
  self.C3AudioFilterFX = class extends c {
    ...
    SetParam(a, b, e, h) {
      switch (a) {
        ...
        case 4:
          ((this._params[4] = b),
            this.SetAudioParam(this._filterNode.gain, b, e, h));
      }
    }
  };
  self.C3AudioDelayFX = class extends c {
    ...
    SetParam(a, b, e, h) {
      ...
      switch (a) {
        ...
        case 5:
          ((this._params[0] = b),
            this.SetAudioParam(this._delayNode.delayTime, b, e, h));
      }
    }
  };
      ...
      switch (a) {
        ...
        case 8:
          ((this._params[3] = b / 100),
            this.SetAudioParam(this._feedbackNode.gain, b / 100, e, h));
      }
    }
  };
  ...
  self.C3AudioGainFX = class extends c {
    ...
    SetParam(a, b, e, h) {
      ...
      switch (a) {
        case 4:
          ((this._params[0] = m(b)),
            this.SetAudioParam(this._node.gain, m(b), e, h));
      }
    }
  };
  ...
  class extends self.DOMHandler {
    ...
    _OnReadyForSWMessages() {
      window.C3_RegisterSW &&
        ...
        window.OfflineClientInfo.SetMessageCallback((c) =>
    ...
    _OnRequestFullscreen(c) {
      if (
        ...
        1 === c
          ? (a.navigationUI = "hide")
          : 2 === c && (a.navigationUI = "show");
        c = document.documentElement;
        c.requestFullscreen
          ? c.requestFullscreen(a)
  ...
  class extends self.DOMHandler {
    constructor(c) {
      ...
      this.AddRuntimeMessageHandlers([
        ...
        ["set-window-height", (a) => this._OnSetWindowHeight(a)],
    ...
    Init() {
      ...
      const a = this._CreateNwjsInputElem(
        "c3nwChooseFolderDialog",
        "nwdirectory",
      );
      a.onchange = (m) => {
      ...
      setInterval(() => {
        ...
        h !== m &&
          ((h = m),
          this.PostToRuntime("clipboard-change", {
            "clipboard-text": m,
          }));
      }, 1e3);
  ...
  self.RuntimeInterface.AddDOMHandlerClass(
    class extends self.DOMHandler {
      constructor(a) {
        ...
        this.AddRuntimeMessageHandler("vibrate", (b) =>
          this._OnGamepadVibrate(b),
        );
        this.AddRuntimeMessageHandler("reset-vibrate", (b) =>
          this._OnGamepadResetVibrate(b),
        );
        window.addEventListener("gamepadconnected", (b) =>
          this._OnGamepadConnected(b),
        );
        window.addEventListener("gamepaddisconnected", (b) =>
          this._OnGamepadDisconnected(b),
        );
      ...
      _GetActiveGamepads() {
        try {
          ...
          return (
            c ||
              (console.warn("[Construct 3] Failed to access gamepads: ", a),
              (c = !0)),
            []
          );
        }
      ...
      _GetGamepadByIndex(a) {
        for (const b of this._GetActiveGamepads()) if (b.index === a) return b;
        return null;
      }
      ...
      _OnGamepadResetVibrate(a) {
        (a = this._GetGamepadByIndex(a.index)) &&
          (a = a.vibrationActuator) &&
          a.reset &&
          a.reset();
      }
      _OnWindowUnload() {
...
self.RuntimeInterface.AddDOMHandlerClass(
  class extends self.DOMHandler {
    constructor(c) {
      ...
      document.addEventListener("pointerlockerror", (a) =>
  ...
  class extends self.DOMHandler {
    ...
    async _OnRequestPermission(c) {
      ...
      0 === c
        ...
        : 1 === c && (a = await this._RequestMotionPermission());
    ...
    async _RequestOrientationPermission() {
      ...
      try {
        ...
        return (
          console.warn("[Touch] Failed to request orientation permission: ", c),
          !1
        );
      }
    }
    async _RequestMotionPermission() {
      ...
      try {
        ...
        return (
          console.warn("[Touch] Failed to request motion permission: ", c),
          !1
        );
      }
    }
...
self.RuntimeInterface.AddDOMHandlerClass(
  class extends self.DOMHandler {
    ...
    _GetSafeAreaInset() {
      ...
      c = [
        c.getPropertyValue("--temp-sai-top"),
        c.getPropertyValue("--temp-sai-right"),
        c.getPropertyValue("--temp-sai-bottom"),
    ...
    async _OnRequestWakeLock() {
      if (!this._screenWakeLock)
        try {
          ...
          (console.warn(
            "[Construct 3] Failed to acquire screen wake lock: ",
            c,
          ),
    ...
    class extends self.DOMElementHandler {
      ...
      CreateElement(a, b) {
        ...
        e.addEventListener("input", () =>
          this.PostToRuntimeElement("input", a, {
            value: parseFloat(e.value),
          }),
        );
        b.id && (e.id = b.id);
