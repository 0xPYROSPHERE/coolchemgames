...
var colorsKeys = [
	...
	// level 43
	[
	 new colorsKey(15, 5,5)
	],
	// level 44
	[
	  new colorsKey(28, 18,6)
	],
	// level 45
	[
	],
...
function submitSavedColorKeys() {
	...
	localStorage.setItem("whg4_colorKeys", JSON.stringify(colorKeysSave));
}

function loadSavedColorKeys() {
	resetColorKeys(realLevel);
	keysSave = JSON.parse(localStorage["whg4_colorKeys"]);
	var t = 0;
	if (keysSave[0] >= 0) {
		for (var i = 0; i < colorsKeys.length; i++) {
			if(colorsKeys[i].length>0){
