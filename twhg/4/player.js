var player = {
	...
	levelOpened:[true,false,false,false,false,false,false,false,false,false,false,false,   true,false,false,false,false,false,false,false,false,false,false,false,   true,false,false,false,false,false,false,false,false,false,false,false,   true,false,false,false,false,false,false,false,false,false,false,false],
	initLevelOpened:[true,false,false,false,false,false,false,false,false,false,false,false,   true,false,false,false,false,false,false,false,false,false,false,false,   true,false,false,false,false,false,false,false,false,false,false,false,   true,false,false,false,false,false,false,false,false,false,false,false],
...
function gotoOtherLevel(){
	if(player.gotoThisLevel!=0){
		...
		localStorage.setItem("whg4_opened", player.levelOpened);
		
		//player.doorPause = false;
		//curCheck = 0;
        //resetEnemies(i + 1);
		if((player.comeBackLevel == 0)&&(player.backLevel!= level)) {
		 resetEnemies(level);
		 resetLifts(level);
		 player.backLevel = level;
	    }
...
function playerMove() {
	if ((state == "game" || state == "main_menu") && !player.dying && (!player.bouncing || player.bounceState >= 4)) {
		...
		*/
		...
		if (keyRight) {
			...
			if(player.iced == true)
				distance = PLAYER_SPEED_ICE;
			else if(player.watered == true){
			    distance = PLAYER_SPEED_WATER;	
			}
		...
		//
		player.liftX += player.arrowX ;
		player.liftY += player.arrowY;
        // right
		
	
		if (player.liftX > 0) {
			distance = player.liftX;
		...
		else if (player.liftX<0) {
			distance = player.liftX * -1;
		...
		// down
	    else if (player.liftY > 0) {
			distance = player.liftY;
	...
	//player.y =  Math.round(player.y* 100000) / 100000;
...
function playerHitLift() {
	getPlayerBounds();
	player.lifted = false;
	if ((state == "game" || state == "main_menu") && !invincible && !invincible_permanent && !playerFullOnCheck() && !player.dying && (!player.bouncing || player.bounceState >= 4)) {
		for (var i = 0; i < lifts[level].length; i++) {
			if( ((lifts[level][i].x) <= player.x) && ((lifts[level][i].x + TILE_SIZE) >= player.x) && ((lifts[level][i].y) <= (player.y - PLAYER_SIZE/2) ) && ((lifts[level][i].y + TILE_SIZE) >= (player.y - PLAYER_SIZE/2) )  )
			/*if (lift[level][i].x + 1> player.left &&
				lift[level][i].x - 1 < player.right &&
				lift[level][i].y + 1 > player.top &&
				lift[level][i].y - 1 < player.bottom) */
			{  
				//player.dying = true;
				//playSFX(sfx_die);
				player.lifted = true;
				lifts[level][i].playerConnected = true;
                player.lift = lifts[level][i]; 
				
				
				switch(lifts[level][i].ground){
					case "ice":
					 player.iced = true;
			         player.watered = false;
			         player.waterCounter = 0;
			         player.arrowY = 0;
                     player.arrowX = 0;
					break;
					...
					case "water":
                     ...
                     if(player.waterCounter>=300){
				       player.dying = true;
					   player.waterdying = true;
					   player.waterCounter = 300;
					   playSFX(sfx_die_water);
					  // player.waterCounter = 300;
			         }
				...
				lifts[level][i].playerConnected = false;
			}
...
function playerHitGround() {
	getPlayerBounds();
	if ((state == "game" || state == "main_menu") && !invincible && !invincible_permanent  && !player.dying && (!player.bouncing || player.bounceState >= 4)) {
		
		 pX = Math.floor(player.x/TILE_SIZE);
		 pY = Math.floor((player.y - PLAYER_SIZE/2)/TILE_SIZE);
         if(player.lifted == true)
             return;			 
        switch(grounds[level][pY][pX]){
			case 1: //black hole
			   if(player.lifted == false){
				 if(player.holeCount>1){  
			      player.dying = true;
				  playSFX(sfx_die_pit);
				  player.darkdying = true;
				  player.waterCounter = 0;
				 }
				 else 
					 player.holeCount++;
...
function playerRespawn() {
	if (state == "game" || state == "main_menu") {
		if (!player.dying && player.alpha < 1 && !finishLevelTimer > 0) {
			...
			player.alpha += RESPAWN_FADE_SPEED*2;
			player.waterCounter = 0;
			if(player.darkdying == true){
			 player.width = 1;
		     player.height = 1;
			 player.darkdying = false;
			}
			if(player.alpha>1){
				player.alpha =1;
				player.backLevel = level;
...
function playerDie() {
	if ((state == "game" || state == "main_menu") && player.dying) {
		if (player.alpha > 0) {
			...
			player.alpha -= DIE_FADE_SPEED*2;
			
			if(player.darkdying == true){
				player.width = player.alpha;
				player.height = player.alpha;
		...
		else
		{
			...
			player.backLevel = 0;
			level = curCheckLevel;
			localStorage.setItem("whg4_deaths", deaths);
			localStorage.setItem("whg4_gameTimer", gameTimer);
			player.alpha = 0;
			player.dying = false;
			checkFlashAlpha = 1;
				...
				if ((player.right > checkpoints[level][i][0] * TILE_SIZE && player.left < (checkpoints[level][i][0] + checkpoints[level][i][2]) * TILE_SIZE) &&
				 ...
				 for (var t = 0; t< levelsNumbers[realLevel].length;t++ ){
		           lev = levelsNumbers[realLevel][t];
					...
					if(level!=0){
					 if (!justWonLevel) {
						localStorage.setItem("whg4_curCheck", curCheck);
						localStorage.setItem("whg4_gameTimer", gameTimer);
						localStorage.setItem("whg4_level", level);
						localStorage.setItem("whg4_realLevel", realLevel);
						...
						if (level < TOTAL_LEVELS)
							localStorage.setItem("whg4_level", startLevels[realLevel]);
						    localStorage.setItem("whg4_realLevel", realLevel+1);
						localStorage.setItem("whg4_curCheck", 0);
						localStorage.setItem("whg4_coins", "[-99]");
						localStorage.setItem("whg4_keys", "[-99]");
						localStorage.setItem("whg4_doors", "[-99]");
						localStorage.setItem("whg4_colorKeys", "[-99]");
    ...
    if ((state == "game" || state == "main_menu") && invincible && finishLevelTimer > 0) {
        ...
        if (finishLevelTimer <= 0) {
	        ...
	        if (realLevel < 4) {
				...
				doorsSave = [-99];
		        localStorage.setItem("whg4_gameTimer", gameTimer);
		        //localStorage.setItem("whg4_level", realLevel);
				
				localStorage.setItem("whg4_level", startLevels[realLevel-1]);
			    localStorage.setItem("whg4_realLevel", realLevel);
				curCheckLevel = level;
		        localStorage.setItem("whg4_deaths", deaths);
		        localStorage.setItem("whg4_curCheck", curCheck);
...
function addCollorToCollect(col) {
	...
	localStorage.setItem("whg4_colors", player.colorsCollected);
	localStorage.setItem("whg4_color", col);
	player.color = col;
...
function drawPlayer() {
	if (state == "game" || state == "main_menu") {
		var drawY = player.y;
		var waterly = 0;
		if(player.waterCounter>=0)
			 waterly = player.waterCounter/300;
		if (player.bouncing)
		...
		// outline
		canvas.beginPath();
		canvas.rect(cwh(player.x - PLAYER_SIZE / 2 * player.width) + os.x, cwh(drawY - PLAYER_SIZE * player.height -0.5) + os.y, cwh(OUTLINE_SIZE * player.width), cwh(PLAYER_SIZE * player.height)); // left
		canvas.rect(cwh(player.x - PLAYER_SIZE / 2 * player.width) + os.x, cwh(drawY - PLAYER_SIZE * player.height - 0.5) + os.y, cwh(PLAYER_SIZE * player.width), cwh(OUTLINE_SIZE * player.height)); // top
		canvas.rect(cwh(player.x + PLAYER_SIZE / 2 * player.width - OUTLINE_SIZE * player.width +0.5) + os.x, cwh(drawY - PLAYER_SIZE * player.height) + os.y, cwh(OUTLINE_SIZE * player.width), cwh(PLAYER_SIZE * player.height)); // right
		...
		canvas.beginPath();
		canvas.rect(cwh(player.x - PLAYER_SIZE / 2 * player.width) + os.x,
			cwh(drawY - PLAYER_SIZE + PLAYER_SIZE * (1-waterly)  ) + os.y,
			cwh(PLAYER_SIZE * player.width),
			cwh(PLAYER_SIZE * player.height * waterly ));
